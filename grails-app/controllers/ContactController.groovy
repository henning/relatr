    ...
    def update = {
        ...
        if(contactInstance) {
            ...
            if(!contactInstance.hasErrors() && contactInstance.save()) {
                // FIXME: actually, we must also check this tag thing for errors!!
                // mean we have to save first, then save strings, and then check errors for tagging!
                contactInstance.parseTags(params.tagsString)
